...
fetch(breeds_URL)
    ...
    .then(data => {
        ...
        for (let i=0; i < breedsArray.length; i++){
            ...
            select.appendChild(option) //adds a nested child to the html element with the option tag
        }
    ...
    const doggoInfo = {
        ...
        assignSneakFood(){
            this.sneakFood = this.yatesShuffle(this.sneakFoodList).slice(0,1)
            document.getElementById('sneakFood').innerHTML = `Will steal food: ${this.sneakFood}`
        },
